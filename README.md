...
Stub DLLs are generated by `scripts/generate_stubs.py`, which reads export names from `data/stubs/exports.json` and uses [LIEF](https://lief-project.github.io/) to build minimal PE32 binaries. Each stub gets a deterministic unique preferred image base so IDA's PE-mode image builder does not collide imported DLLs on non-Windows hosts. No network access is needed at build time. To update the export list (e.g. for a newer Wine release), regenerate the JSON from Wine `.spec` files and commit it.
